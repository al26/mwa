...
<?php if (empty($recent_posts[0]->category)): ?>
	<h3>Tidak ada berita untuk <?=$this->uri->segment(2)." ".ucwords(str_replace("-"," ",$this->uri->segment(3)));?></h3>
<?php else : ?>
<?php foreach ($recent_posts as $post) : ?>
<div class="post-container">
<h2 class="no-margin"><?= ucwords($post->title) ?></h2>
<h5><span class="glyphicon glyphicon-time"></span> <?= "Dipublikasikan pada ".date_format(date_create($post->created_at), 'j/m/Y'); ?> | kategori :<?php $categories = explode(",",$post->category); 
  if (!empty($categories)) { 
