<section class="content">
  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Perbarui Konten Utama Halaman <?=ucwords($data->title);?></h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          <form class="form-horizontal" enctype="multipart/form-data" method="post" action="<?=base_url('kelola-fk');?>">
            <input type="hidden" name="id" value="<?=$fk->id;?>">
            <div class="form-group">
              <label class="control-label col-sm-3 col-md-2 color-aqua" for="title">Fungsi </label>
              <div class="col-sm-8 col-md-9"> 
                <textarea class="textarea" rows="6" name="fungsi" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required><?=(!empty($fk->fungsi)) ? $fk->fungsi : $this->session->flashdata('fungsi');?></textarea>
              </div>
            </div>
            <div class="form-group">
            <label class="control-label col-sm-3 col-md-2 color-aqua">Kewenangan</label>
            <div class="col-sm-8 col-md-9">
              <textarea class="textarea" rows="6" name="kewenangan" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required><?=(!empty($fk->kewenangan)) ? $fk->kewenangan : $this->session->flashdata('kewenangan');?></textarea>
            </div>
            </div>
            <div class="form-group"> 
              <div class="col-sm-offset-3 col-md-offset-2 col-sm-10">
